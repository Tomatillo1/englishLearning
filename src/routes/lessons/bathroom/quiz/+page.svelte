<script lang="ts">
	...
	const bathRoomQuestions = lessonBathroomItems.lessonQuestions;
	...
	<main class="min-h-screen w-full flex flex-col px-4 justify-center items-center">
		<section class="flex flex-col bg-white border-2 border-gray-100 rounded-2xl w-full max-w-md items-center justify-center py-16 gap-6">
			...
			<div class="flex flex-col w-full gap-6 pt-4 items-center">
				<button onclick={displayResult = undefined} class="flex flex-row justify-center items-center gap-2 w-2/3 text-white text-2xl bg-red-700 rounded-xl px-4 py-2"><RotateCcw class="stroke-white"/>Try again ?</button>
...
{:else}
	<main class="h-screen flex flex-col w-full px-4 pt-4 ">
		...
		<Quiz questionsList={bathRoomQuestions} onQuizFinished={onQuizFinished} />
	</main>
{/if}
